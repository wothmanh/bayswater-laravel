class MyConstractor {
    ...
    accommoMaxWeeks(maxWeeks) {
        console.log("accommoMaxWeeks max ", maxWeeks);
        self.FormAccommodationWeeks.innerHTML = "";
        for(var i = maxWeeks + 2; i >= 0; --i) {
...
//                    [...self.HolderRegistrationFeeVal].map(element => element.innerText = self.centreCurrencyName+' '+self.centreRegistrationFee); 
    ...
    courseWeeksOn(sel) {
        console.log("courseWeeksOn (sel)", sel);
        return new Promise((resolve, reject) => {
            if (sel.value) {
...
class Calcul extends MyForm {
    ...
    calculate() {
        ...
        let subSum2 = this.getSum(self.subSum2T).toFixed(2) + guardAndChrisSum.toFixed(2) + custodianshipTotal.toFixed(2) - DiscountSum2T.toFixed(2);
...
function course_weeks_onchange(sel) {
    console.log("course_weeks_onchange .. ");
