...
class Courses_Family extends Admin_Controller
{
	...
	public function index ($country_id = 'all', $city_id = 'all', $school_id = 'all')
	{
        ...
        $paginate_info = paginate_it(
			'courses_family/index/'.$country_id.'/'.$city_id.'/'.$school_id, 20, 
			$this->clients_m->count_this('courses_family',$erwhere), 
			6, 
			''
		); 
        $this->data['courses_family'] = $this->courses_family_m->get_limit($paginate_info["per_page"], $paginate_info['page'], $erwhere);
        $this->data['links'] = $paginate_info['links'];

		$this->data['subview'] = 'admin/courses_family/index';
	...
	{
		if ($id) {
			$this->data['courses_family'] = $this->courses_family_m->get($id);
			!is_null($this->data['courses_family']) || $this->data['errors'][] = 'courses add-ons could not be found';
		}
		else {
			$this->data['courses_family'] = $this->courses_family_m->get_new();
		}
		...
		$rules = $this->courses_family_m->rules;
		$this->form_validation->set_rules($rules);
		
		if ($this->form_validation->run() == TRUE) {
			$data = $this->courses_family_m->array_from_post(array(
				'name', 'lessons', 'age_from', 'age_to', 'price', 'extra', 'date_from', 'date_to', 'notes', 'active', 'school_id'
			));
			...
			$this->courses_family_m->save($data, $id);
			redirect('courses_family');
		}
		
		$this->data['subview'] = 'admin/courses_family/edit';
		$this->load->view('admin/_layout_main', $this->data);
	}

	public function delete ($id)
	{
		$this->courses_family_m->delete($id);
		redirect('courses_family');
	}
