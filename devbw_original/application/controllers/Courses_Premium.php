...
class Courses_Premium extends Admin_Controller
{
	...
	public function index ($country_id = 'all', $city_id = 'all', $school_id = 'all')
	{
        ...
        $this->data['courses_premium'] = $this->courses_premium_m->get_limit($paginate_info["per_page"], $paginate_info['page'], $erwhere);
        $this->data['links'] = $paginate_info['links'];

		$this->data['subview'] = 'admin/courses_premium/index';
	...
	{
		if ($id) {
			$this->data['courses_family'] = $this->courses_premium_m->get($id);
			!is_null($this->data['courses_family']) || $this->data['errors'][] = 'courses add-ons could not be found';
		}
		else {
			$this->data['courses_family'] = $this->courses_premium_m->get_new();
		}
		...
		$rules = $this->courses_premium_m->rules;
		$this->form_validation->set_rules($rules);
		
		if ($this->form_validation->run() == TRUE) {
			$data = $this->courses_premium_m->array_from_post(array(
				'name', 'notes', 'active', 'school_id'
			));
			...
			$this->courses_premium_m->save($data, $id);
			redirect('courses_premium');
		}
		
		$this->data['subview'] = 'admin/courses_premium/edit';
		$this->load->view('admin/_layout_main', $this->data);
	}

	public function delete ($id)
	{
		$this->courses_premium_m->delete($id);
		redirect('courses_premium');
	}
