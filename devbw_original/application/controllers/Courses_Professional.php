...
class Courses_Professional extends Admin_Controller
{
	...
	public function index ($country_id = 'all', $city_id = 'all', $school_id = 'all')
	{
        ...
        $paginate_info = paginate_it('courses_professional/index/'.$country_id.'/'.$city_id.'/'.$school_id, 20, $this->clients_m->count_this('courses_professional',$erwhere), 6, ''); 
        $this->data['courses'] = $this->courses_professional_m->get_limit($paginate_info["per_page"], $paginate_info['page'], $erwhere);
        $this->data['links'] = $paginate_info['links'];

		$this->data['subview'] = 'admin/courses_professional/index';
	...
	{
		if ($id) {
			$this->data['courses'] = $this->courses_professional_m->get($id);
			!is_null($this->data['courses']) || $this->data['errors'][] = 'courses could not be found';
		}
		else {
			$this->data['courses'] = $this->courses_professional_m->get_new();
		}
		...
		$rules = $this->courses_professional_m->rules;
		$this->form_validation->set_rules($rules);
		
		if ($this->form_validation->run() == TRUE) {
			$data = $this->courses_professional_m->array_from_post(array(
				'name', 'active', 'start_dates', 'school_id', 'notes', 'duration', 'price'
			));
			...
			$this->courses_professional_m->save($data, $id);
			redirect('courses_professional');
		}
		
		$this->data['subview'] = 'admin/courses_professional/edit';
		$this->load->view('admin/_layout_main', $this->data);
	}

	public function delete ($id)
	{
		$this->courses_professional_m->delete($id);
		redirect('courses_professional');
	}
